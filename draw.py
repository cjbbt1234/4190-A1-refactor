...
COLORS = ['floral white', 'brown1', 'LightCyan3', 'indian red', 'SlateBlue3', 'wheat4', 'dark turquoise',
          'LightGoldenrod2', 'tan2', 'RoyalBlue4', 'tomato3', 'LightYellow3', 'gray57', 'khaki4', 'gray53', 'orange red']
    ...
    for index in range(len(blocks)):
        ...
        for p in block:
            ...
            if p in solution:  # if this position is in solution list, then we need to draw star in it
                Label(window, bg=COLORS[index], relief=GROOVE, width=5, height=3, text=star).grid(row=row, column=col)
            else:
                Label(window, bg=COLORS[index], relief=GROOVE, width=5, height=3).grid(row=row, column=col)
    window.mainloop()
